    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex flex-wrap items-center gap-2 mb-1">
          <span
            className={`font-semibold text-sm ${isDark ? 'text-stone-100' : 'text-stone-900'}`}
          >
            {author.name}
          </span>
...
function SourcesList({
  ...
  return (
    <div className={`mt-8 pt-6 border-t ${isDark ? 'border-stone-800' : 'border-stone-200'}`}>
      <p
        className={`text-[11px] font-semibold tracking-widest uppercase mb-4 ${isDark ? 'text-stone-500' : 'text-stone-400'}`}
      >
        References & Sources
      </p>
      <ul className="space-y-2">
        {sources.map((s, i) => (
          <li key={i} className="flex items-baseline gap-3">
            <span
              className={`text-xs font-mono shrink-0 ${isDark ? 'text-stone-700' : 'text-stone-400'}`}
            >
              [{i + 1}]
            </span>
            <a
      ...
      <div className="flex items-start gap-3 mt-1">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span
              className={`text-sm font-semibold transition-colors ${isDark ? 'text-stone-200 group-hover:text-amber-300' : 'text-stone-800 group-hover:text-amber-700'}`}
            >
              {ad.company}
            </span>
            <span
              className={`text-[9px] font-semibold tracking-widest uppercase px-1.5 py-0.5 rounded ${isDark ? 'bg-stone-800 text-stone-600' : 'bg-stone-100 text-stone-400'}`}
            >
              Sponsored
            </span>
          </div>
          <p
            className={`text-xs leading-snug ${isDark ? 'text-stone-500' : 'text-stone-500'}`}
          >
            {ad.tagline}
          </p>
        </div>
...
function ReadingProgress({ theme }: { theme: Theme }) {
  ...
  return (
    <div
      className={`fixed top-0 left-0 right-0 z-50 h-[3px] ${theme === 'dark' ? 'bg-stone-900' : 'bg-stone-200'}`}
    >
      <div
...
export default function BlogDetailPage({ params }: PageProps) {
  ...
  if (!post) {
    return (
      <div
        className={`min-h-screen flex flex-col items-center justify-center gap-4 ${isDark ? 'bg-stone-950 text-stone-400' : 'bg-stone-50 text-stone-500'}`}
      >
  ...
  return (
    ...
    >
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <span
              className={`text-sm font-medium truncate ${isDark ? 'text-stone-400' : 'text-stone-600'}`}
            >
              {post.title}
            </span>
          </div>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12">
        ...
        {/* Subtitle */}
        <p
          className={`text-lg leading-relaxed mb-8 ${isDark ? 'text-stone-400' : 'text-stone-600'}`}
        >
          {post.subtitle}
        </p>
        ...
        {/* Meta row */}
        <div
          className={`flex flex-wrap items-center gap-4 py-4 border-y mb-10 ${isDark ? 'border-stone-800' : 'border-stone-200'}`}
        >
          <div className="flex items-center gap-2">
            <div className="flex -space-x-2">
              {post.authors.map((a) => (
                <AuthorPip key={a.id} author={a} size="sm" />
              ))}
            </div>
            <span
        ...
        <div className="flex flex-wrap gap-2 mt-8">
          {post.tags.map((t) => (
            <span
              key={t}
              className={`text-[11px] border px-3 py-1 rounded-full ${isDark ? 'text-stone-500 border-stone-800' : 'text-stone-500 border-stone-200'}`}
            >
              #{t}
            </span>
          ))}
        ...
        {/* ── Author Bios ── */}
        <div
          className={`mt-10 pt-8 border-t ${isDark ? 'border-stone-800' : 'border-stone-200'}`}
        >
          <p
            className={`text-[11px] font-semibold tracking-widest uppercase mb-4 ${isDark ? 'text-stone-500' : 'text-stone-400'}`}
          >
        ...
        {/* ── Back link ── */}
        <div
          className={`mt-10 pt-6 border-t ${isDark ? 'border-stone-900' : 'border-stone-200'}`}
        >
          <button
            onClick={() => router.push('/blog')}
      ...
      {/* ────────────────────────────────────────────────────────────────────── */}
      {/* ── Sponsored Section (lowest priority, clearly labelled) ── */}
      {/* ────────────────────────────────────────────────────────────────────── */}
      <div
        className={`border-t mt-4 ${isDark ? 'border-stone-900' : 'border-stone-200'}`}
      >
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
          {/* Label */}
          <div className="flex items-center gap-3 mb-6">
            <p
              className={`text-[10px] font-semibold tracking-widest uppercase ${isDark ? 'text-stone-700' : 'text-stone-400'}`}
            >
              From our partners — related to{' '}
              <span className={isDark ? 'text-stone-600' : 'text-stone-500'}>
                {post.category}
              </span>
            </p>
            <div
              className={`flex-1 h-px ${isDark ? 'bg-stone-900' : 'bg-stone-200'}`}
            />
          </div>
          ...
          {/* Advertise CTA */}
          <p
            className={`text-center text-[11px] mt-6 ${isDark ? 'text-stone-800' : 'text-stone-300'}`}
          >
            Want to reach architects, planners &amp; built environment
            professionals?{' '}
            <a
              href="mailto:ads@modelblog.example"
              className={`underline underline-offset-2 transition-colors ${isDark ? 'hover:text-stone-500' : 'hover:text-stone-500'}`}
            >
      ...
      >
        ModelBlog — informed writing on architecture, cities, and the built
        environment. All articles are independently authored and editorially
        reviewed.
      </footer>
    </div>
  );
