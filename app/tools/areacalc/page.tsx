...
function DimAreaInput({
  ...
  function handleL(val: string) {
    onUpdate({ ...space, L: dimFromUnit(parseFloat(val) || 1, unit) });
  }
  function handleB(val: string) {
    onUpdate({ ...space, B: dimFromUnit(parseFloat(val) || 1, unit) });
  }
  function confirmArea() {
    const target = parseFloat(areaDraft);
    const current = parseFloat(fmt(space.L * space.B, unit).replace(/,/g, ""));
    ...
    onUpdate({
      ...space,
      L: Math.max(0.01, space.L * ratio),
      B: Math.max(0.01, space.B * ratio),
    });
    setAreaError("");
  }
      ...
      <div>
        ...
        <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
          <input
            ...
            style={{ width: 104, fontSize: 13, padding: "5px 8px", borderRadius: 6, border: areaError ? "1px solid #ef4444" : "1px solid #e5e7eb" }}
          />
          <button type="button" onClick={confirmArea} style={{ height: 30, minWidth: 32, borderRadius: 6, border: "1px solid #16a34a", background: "#dcfce7", color: "#166534", cursor: "pointer", fontWeight: 800 }}>✓</button>
        </div>
...
function SubSpaceRow({
  ...
  function confirmArea() {
    const target = parseFloat(areaDraft);
    const current = parseFloat(fmt(sub.L * sub.B, unit).replace(/,/g, ""));
  ...
  return (
    <div style={{ marginLeft: 16, marginTop: 6, display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center", background: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: 8, padding: "8px 12px" }}>
      ...
      <button type="button" onClick={confirmArea} style={{ fontSize: 12, color: "#166534", background: "#dcfce7", border: "1px solid #16a34a", cursor: "pointer", padding: "2px 7px", borderRadius: 5, fontWeight: 800 }}>✓</button>
...
function SummaryCard({
  ...
  unit: UnitKey; wall: number; circ: number; costPerSqft: number;
  spaces: SpaceInstance[]; floorGroups: Map<number, SpaceInstance[]>; locationLabel: string;
}) {
  const aLabel = UNIT_SYSTEMS[unit].areaLabel;
  return (
...
export default function App() {
  ...
  const locationLabel = [selectedCountry?.name, selectedState?.name, selectedPlace?.name]
    .filter(Boolean).join(" · ");

  // Load countries + templates on mount
  useEffect(() => {
          ...
          <div style={{ background: "#fffbeb", borderTop: "1px solid #fde68a", padding: "12px 16px" }}>
            <div style={{ maxWidth: 1400, margin: "0 auto", display: "flex", flexWrap: "wrap", gap: 20, alignItems: "flex-end" }}>
              ...
              <div>
                ...
                <select value={countryId ?? ""} onChange={(e) => setCountryId(e.target.value ? Number(e.target.value) : null)} style={{ fontSize: 13, padding: "6px 10px", borderRadius: 8, border: "1px solid #e5e7eb", background: "#fff", minWidth: 140 }}>
              ...
              <div>
                ...
                <select value={stateId ?? ""} onChange={(e) => setStateId(e.target.value ? Number(e.target.value) : null)} disabled={!countryId || statesList.length === 0} style={{ fontSize: 13, padding: "6px 10px", borderRadius: 8, border: "1px solid #e5e7eb", background: "#fff", minWidth: 160 }}>
                  <option value="">— Select —</option>
              ...
              <div>
                ...
                <select value={placeId ?? ""} onChange={(e) => setPlaceId(e.target.value ? Number(e.target.value) : null)} disabled={!stateId || placesList.length === 0} style={{ fontSize: 13, padding: "6px 10px", borderRadius: 8, border: "1px solid #e5e7eb", background: "#fff", minWidth: 180 }}>
                  <option value="">— Select —</option>
              ...
              <div>
                ...
                <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                  ...
                  <input type="range" min={0} max={50} step={1} value={wall} onChange={(e) => setWall(parseInt(e.target.value))} style={{ width: 90, accentColor: "#f59e0b" }} />
                </div>
              </div>
              ...
              <div style={{ fontSize: 12, color: "#6b7280", borderLeft: "2px solid #fcd34d", paddingLeft: 12 }}>
                ...
                {customRate && (
                  <span style={{ marginLeft: 6, fontSize: 10, fontWeight: 700, padding: "2px 6px", borderRadius: 20, background: "#fef3c7", color: "#92400e", border: "1px solid #fcd34d" }}>Custom override</span>
                )}
                <br />
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "16px", display: "flex", gap: 16, alignItems: "flex-start" }}>
        <main style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
            <button onClick={() => setPaletteOpen(true)} style={{ fontSize: 13, padding: "8px 14px", borderRadius: 8, border: "1px solid #e5e7eb", background: "#fff", cursor: "pointer", fontWeight: 600, color: "#374151" }}>
          ...
          {visibleSpaces.length === 0 ? (
            ...
            Array.from(floorGroups.entries()).map(([floor, fs]) => (
              <div key={floor} style={{ marginBottom: 20 }}>
                ...
                {fs.map((s) => <SpaceCard key={s.instanceId} space={s} unit={unit} spaceTemplates={spaceTemplates} onUpdate={(u) => updateSpace(s.instanceId, u)} onRemove={() => removeSpace(s.instanceId)} />)}
              </div>
            ))
          ) : (
            visibleSpaces.map((s) => <SpaceCard key={s.instanceId} space={s} unit={unit} spaceTemplates={spaceTemplates} onUpdate={(u) => updateSpace(s.instanceId, u)} onRemove={() => removeSpace(s.instanceId)} />)
          )}

          {spaces.length > 0 && (
            <SummaryCard totals={totals} unit={unit} wall={wall} circ={circ} costPerSqft={costPerSqft} spaces={spaces} floorGroups={floorGroups} locationLabel={locationLabel} />
          )}
        </main>
      </div>
