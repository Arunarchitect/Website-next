...
export interface MemberWorkLogEntry {
  id: number;
  organisation_id: number; organisation_name: string;
  project_id: number; project_name: string;
  deliverable: number; deliverable_name: string;
  employee_name: string;
  start_time: string; start_time_fmt: string; start_date_fmt: string;
  end_time: string | null; end_time_fmt: string | null; end_date_fmt: string | null;
  remarks: string | null; finalised: boolean;
}

export interface WorkLogPage {
  count: number; page: number; pages: number; results: MemberWorkLogEntry[];
}
...
export interface AssignmentEntry {
  ...
  deliverable_id: number; deliverable_name: string;
  project_id: number; project_name: string;
  organisation_id: number; organisation_name: string;
  assigned_to_id: number; assigned_to_name: string;
...
export async function fetchMemberWorkLogs(params: {
  member_id?: number; from?: string; to?: string; page?: number;
}): Promise<WorkLogPage> {
...
export async function fetchAssignments(memberId?: number): Promise<AssignmentEntry[]> {
...
export async function createAssignment(payload: {
  name: string; deliverable: number; assigned_to: number;
...
export async function editAssignment(id: number, payload: {
  name?: string; deliverable?: number; assigned_to?: number;
  start_date?: string | null; due_date?: string | null;
}): Promise<AssignmentEntry> {
