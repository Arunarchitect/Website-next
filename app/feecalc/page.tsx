...
function generatePDF(params: {
  ...
  if (activeTypes.hourly && hourlyResult) {
    ...
    if (hourlyResult.consultant_rates.length > 0) {
      ...
      autoTable(doc, {
        startY: y,
        // ── CHANGED: removed "Effective From" and "Until" columns ──
        head: [["Role / Designation", "Rate / Hour"]],
        body: hourlyResult.consultant_rates.map((r) => {
          // ── CHANGED: use normaliseRole so empty string → null ──
      ...
      {data.consultant_rates.length > 0 && (
        ...
        >
          ...
          {data.consultant_rates.map((r, i) => {
            // ── CHANGED: normaliseRole handles empty string → null ──
            const role = normaliseRole(r.role);
            ...
            return (
              ...
              >
                ...
                {/* Role — ── CHANGED: no date line below ── */}
                <div style={{ flex: 1, minWidth: 0 }}>
                  {role ? (
